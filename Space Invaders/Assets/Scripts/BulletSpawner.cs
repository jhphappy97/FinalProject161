...
public class BulletSpawner : MonoBehaviour
{
    ...
    void fireBullet()
    {
        if (ableToFire)
        {
            print("FiredBullet");
            ++bulletCount;
            Rigidbody2D bulletClone = (Rigidbody2D)Instantiate(bullet, new Vector3(player.GetComponent<Rigidbody2D>().transform.position.x, player.GetComponent<Rigidbody2D>().transform.position.y + 0.8f,0), Quaternion.Euler(0,0,90f));
